...
use tokio_postgres::{Error, Row, Statement, ToStatement, Column, Type};
...
impl MockClient {
  ...
  pub async fn query_opt(&self, _query: &str, _params: &[&(dyn tokio_postgres::types::ToSql + Sync)]) -> Result<Option<Row>, Error> {
  ...
  pub async fn query_one(&self, _query: &str, _params: &[&(dyn tokio_postgres::types::ToSql + Sync)]) -> Result<Row, Error> {
    if !self.sequential_clients.lock().unwrap().is_empty() {
      let current = *self.current_call.lock().unwrap();
      let clients = self.sequential_clients.lock().unwrap();
      if current < clients.len() {
  ...
  pub async fn query(&self, _query: &str, _params: &[&(dyn tokio_postgres::types::ToSql + Sync)]) -> Result<Vec<Row>, Error> {
    if !self.sequential_clients.lock().unwrap().is_empty() {
      let current = *self.current_call.lock().unwrap();
      let clients = self.sequential_clients.lock().unwrap();
      if current < clients.len() {
  ...
  pub async fn execute(&self, _query: &str, _params: &[&(dyn tokio_postgres::types::ToSql + Sync)]) -> Result<u64, Error> {
    if !self.sequential_clients.lock().unwrap().is_empty() {
      let current = *self.current_call.lock().unwrap();
      let clients = self.sequential_clients.lock().unwrap();
      if current < clients.len() {
  ...
  pub async fn execute_raw<T: ToStatement + Sync>(&self, _statement: &T, _params: &[&(dyn tokio_postgres::types::ToSql + Sync)]) -> Result<u64, Error> {
  ...
  pub async fn query_raw<T: ToStatement + Sync>(&self, _statement: &T, _params: &[&(dyn tokio_postgres::types::ToSql + Sync)]) -> Result<Vec<Row>, Error> {
...
pub fn create_mock_user_row(id: &str, provider: String, provider_user_id: &str) -> Row {
  let column_names = vec!["id", "provider", "provider_user_id"];
  let column_types = vec![Type::TEXT, Type::TEXT, Type::TEXT];
  let columns: Vec<Column> = column_names.iter().enumerate()
