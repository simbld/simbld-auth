...
use rand::rngs::OsRng;
use simbld_http::responses::local::{hashing_error, invalid_password};
...
impl UserService {
  ...
  pub async fn count_users(client: &Client) -> Result<i64, UserError> {
    UserRepositoryImpl::count(client).await
  }
  ...
  pub async fn get_users_with_pagination(
    ...
    UserRepositoryImpl::get_with_pagination(client, limit, offset).await
  }
  ...
  pub fn verify_password(hash: &str, password: &str) -> Result<bool, UserError> {
    let parsed_hash = PasswordHash::new(hash).map_err(|_| hashing_error)?;
  ...
  fn hash_password(password: &str) -> Result<String, UserError> {
    ...
    let argon2 = Argon2::default();
    let password_hash =
      argon2.hash_password(password.as_bytes(), &salt).map_err(|_| hashing_error)?;
