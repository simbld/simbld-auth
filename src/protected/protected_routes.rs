...
use simbld_http::responses::success::{authentication_successful, ok};
...
async fn extract_auth_user(req: &HttpRequest) -> Result<AuthenticatedUser, HttpResponse> {
    ...
    if !auth_header.starts_with("Bearer ") {
        ...
        return Err(HttpResponse::build(StatusCode::from_u16(code).unwrap()).body(description));
    }
    ...
    // Validate token
    let email = AuthService::validate_token(token).await.map_err(|(code, desc)| {
        HttpResponse::build(StatusCode::from_u16(code).unwrap()).body(desc)
    })?;
...
async fn protected_route(req: HttpRequest) -> impl Responder {
    ...
    HttpResponse::build(StatusCode::from_u16(code).unwrap()).json(json!({
        "code": code,
        "desc": description,
        "data": {
...
pub fn configure_protected_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/protected")
            ...
            .route("/profile", web::get().to(get_user_profile)), // Additional protected routes can be added here
    );
}
