...
pub fn map_jwt_error(err: JwtError) -> crate::errors::ApiError {
    ...
    match err.kind() {
        jsonwebtoken::errors::ErrorKind::ExpiredSignature => {
            ApiError::new(401, "Token has expired".to_string())
        },
        jsonwebtoken::errors::ErrorKind::InvalidToken => {
            ApiError::new(401, "Invalid token format".to_string())
        },
        jsonwebtoken::errors::ErrorKind::InvalidSignature => {
            ApiError::new(401, "Invalid token signature".to_string())
        },
        _ => ApiError::new(401, "Authentication error".to_string()),
    }
}
...
pub fn extract_claims_from_request(
    ...
    let auth_header = req
        ...
        .get("Authorization")
        .ok_or_else(|| {
            crate::errors::ApiError::new(401, "No authorization header found".to_string())
        })?
        .to_str()
        .map_err(|_| {
            crate::errors::ApiError::new(401, "Invalid authorization header".to_string())
        })?;

    // Check if it's a Bearer token
    if !auth_header.starts_with("Bearer ") {
        return Err(crate::errors::ApiError::new(401, "Invalid authentication scheme".to_string()));
    }
    ...
    // Verify the token
    jwt_manager.verify_token(token).map_err(map_jwt_error)
}
