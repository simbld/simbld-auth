...
impl PasswordResetToken {
    ...
    pub async fn create_token(
        ...
        let _result = client
            .execute("DELETE FROM password_reset_tokens WHERE user_id = $1", &[&user_id])
            .await
    ...
    pub async fn check_token(
        client: &Client,
        token: &str,
    ) -> Result<Option<Self>, PasswordResetError> {
        let now = Utc::now();
    ...
    pub async fn mark_as_used(client: &Client, token: &str) -> Result<(), PasswordResetError> {
        let result = client
            .execute("UPDATE password_reset_tokens SET used = true WHERE token = $1", &[&token])
            .await
            .map_err(|e| PasswordResetError::DatabaseError(e.to_string()))?;
    ...
    pub async fn clean_expired_tokens(client: &Client) -> Result<u64, PasswordResetError> {
        ...
        let result = client
            .execute("DELETE FROM password_reset_tokens WHERE expires_at < $1", &[&now])
            .await
            .map_err(|e| PasswordResetError::DatabaseError(e.to_string()))?;
...
impl PasswordRotationManager {
    ...
    pub async fn update_password_changed(
        client: &Client,
        user_id: Uuid,
        ...
        let _result = client
            .execute("UPDATE users SET password_last_changed = $1 WHERE id = $2", &[&now, &user_id])
            .await
            .map_err(|e| PasswordResetError::DatabaseError(e.to_string()))?;
    ...
    pub async fn get_users_needing_rotation(
        client: &Client,
    ) -> Result<Vec<Uuid>, PasswordResetError> {
        ...
        let user_ids = rows.iter().map(|row| row.get::<_, Uuid>("id")).collect();
...
impl PasswordResetNotifier {
    ...
    pub async fn send_rotation_reminder(
        email: &str,
        days_remaining: i64,
    ) -> Result<(), PasswordResetError> {
        // This would integrate with your email service
